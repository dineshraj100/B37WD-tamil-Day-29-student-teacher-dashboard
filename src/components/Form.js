...
const FormFill = () => {
  ...
  const [showLeafIcon, setShowLeafIcon] = useState(
    state != null ? state.swift : false
  );
  ...
  const idstud =
    lastIndexstud === -1 ? 1 : contextData.students[lastIndexstud].id + 1;
  const idteach =
    lastIndexteach === -1 ? 200 : contextData.teachers[lastIndexteach].id + 1;
    ...
    if (state === null) {
      if (showLeafIcon === true) {
        contextData.setStudents([
          ...
          { ...values, id: idstud, who: 'student' },
        ]);
        toast.success('Student Added Successfully');
      } else {
        contextData.setTeachers([
          ...contextData.teachers,
          { ...values, id: idteach, who: 'teacher' },
        ]);
      ...
      console.log(values);
      if (showLeafIcon === true) {
        const finalUpdatedstuds = contextData.students.map((e) =>
          e.id === values.id ? { ...values, who: 'student' } : e
        );
        contextData.setStudents(finalUpdatedstuds);
        navigate('/students');
        toast.success('Updated successfully');
      } else {
        const finalUpdatedtchrs = contextData.teachers.map((e) =>
          e.id === values.id ? { ...values, who: 'teacher' } : e
        );
  ...
  return (
    <>
      ...
      >
        <div
          className='container'
          style={{ padding: '0 18%', alignItems: 'left' }}
        >
          ...
          >
            <Input
              disabled={
                state === null ? false : state.action === 'view' ? true : false
              }
            />
          </Form.Item>
